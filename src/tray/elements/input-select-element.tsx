...
import IState, { InputSelectAttributes } from '../../types/state';
import { sendHeight } from '../send-height';
import ElementIcon from './element-icon';

interface InputSelectElementProps {
  state: IState<InputSelectAttributes>
  entity: IEntityConfig
  refetch: () => void
}

export default function InputSelectElement(props: InputSelectElementProps) {
        ...
        <div className="max-h-64 overflow-x-auto">
          {state.attributes.options.map((option) => (
            <button
              ...
              onClick={async () => {
                await window.electronAPI.state.callServiceAction('input_select', 'select_option', { entity_id: entity.entity_id, option });
                await refetch();
              }}
            >
              {option}
